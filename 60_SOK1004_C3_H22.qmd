...
df_wide <- df_wide %>%
  group_by(dato) %>%
  summarise(across(c("verdi","Konsumprisindeks (vekter)","sjekk <- sum(verdi)"), mean, na.ie = TRUE))
```
...
Det vi ser på grafen er Elektrisitet inkludert i nettleie. Dette er den direkte effekten av kraftpriser. Dette er gjennom husholdningers forbruk. Vi ser at den gjennomsnittlige verdien av Elektrisitet inkludert nettleie er den blå linja som ligger rundt 75 og går opp mot 100 etterhvert som årene går.
...
Ved høye indirekte priser kan dette være med på å sette den direkte prisen opp. Dette er fordi når den indirekte prisen stiger kan dette være med på å påvirke den direkte prisen i senere tid.
