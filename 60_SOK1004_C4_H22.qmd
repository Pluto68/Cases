...
Jeg vil si at binwidth på rundt 1 er ganske greit da ser man greit verdiene på grafen. Hvis vi tar hensyn på hvor stor grafen skal være når vi ser på 10 eller 30 kan man skille de ganske greit ut når de er mer samlet.
...
df <- rename(df, næring = `næring (SN2007)`)

```

# Oppgave III: Undersøk
...
df_w %>%
ggplot(aes(verdi)) +
geom_histogram(aes(fill = statistikkvariabel),binwidth = 600) +
facet_grid(~næring)
